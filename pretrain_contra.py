...
parser.add_argument(
    "--train_datasets",
    type=str,
    default="ASD,MSL,PSM,SWAT,NIPS_TS_Creditcard",
...
run_dir = "pretrain/" + name_with_datetime(args.save_name)
os.makedirs(run_dir, exist_ok=True)
model = MaskModelInterface(**config)

# pretrain
dataset, data_loader = train_data_provider(
    args.data_path,
    args.train_datasets,
    args.batch_size,
    win_size=args.win_size,
...
t = time.time()
loss_log, loss_log_iters = model.pretrain(
    data_loader, n_epochs=args.epochs, n_iters=args.iters, verbose=True
)
t = time.time() - t
...
result.to_csv(f"{run_dir}/result_epoch.csv", index=False)
